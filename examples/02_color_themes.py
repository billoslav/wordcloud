...
def main() -> None:
    ...
    themes = ["viridis", "plasma", "default"]
    for theme in themes:
        wc = Wordcloud(width=800, height=400, font_path=str(font_path), background_color="white")
        wc.generate(text, color_theme=theme)
        wc.draw_image(save_file=True, image_name=f"02_color_theme_{theme}")
        print(f"Saved Results/02_color_theme_{theme}.png")
